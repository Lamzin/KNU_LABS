...
x0_ = integrate((x1(x)) ** 2, (x, 0, h)) / h

xN_ = integrate((x1(x)) ** 2, (x, R, R - h)) / h


def xi_(i):
    return integrate((x1(x)) ** 2, (x, (i - 1), (i + 1))) / h / 2
...
def get_next_layer(y_prev):
    ...
    c0 = - alpha1 * x0_ / 2 - b0
    phi0 = -tau * x0_ * mu2 / h \
           - alpha1 * x0_ * y_prev[0] / 2 \
           - (1 - sigma) * tau * alpha1 * pi_(1) * (y_prev[1] - y_prev[0]) / h / h

    dN = sigma * tau * alpha2 * pi_(N - 1) / h / h
    cN = - alpha1 * x0_ / 2 - dN
    phiN = (1 - sigma) * tau * beta2 * x1(R - h) * y_prev[N - 1] / h \
           - tau * xN_ * mu2 / h - alpha2 * xN_ * y_prev[N - 1] / 2 \
